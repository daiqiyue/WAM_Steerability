...
def get_libero_env(task, model_family, resolution=256, horizon=1000):
    """Initializes and returns the LIBERO environment, along with the task description.

    `horizon` is the underlying robosuite step cap; the env raises
    "executing action in terminated episode" after this many `env.step`s.
    Default 1000 matches LIBERO's `ControlEnv.__init__` default. Pass a
    larger value when your rollout loop (including any `num_steps_wait`
    no-op steps at episode start) may exceed 1000 total steps."""
