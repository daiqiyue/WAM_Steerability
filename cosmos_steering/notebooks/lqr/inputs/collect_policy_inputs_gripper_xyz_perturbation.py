#!/usr/bin/env python
"""Collect Cosmos-Policy positive/negative policy inputs by running N
gripper-xyz-perturbed rollouts of libero_10 task 0 and bucketing every
captured inference by whether its rollout succeeded or failed.

Scene / prompt / objects are unchanged — the only stress applied is the
RandomGripperXYZPerturbation from 11_gripper_xyz_perturbation.ipynb
(default preset: xyz_random_xlarge_3, sigma=10cm isotropic, per-episode
Gaussian sampled via SeedSequence([base_seed, episode_idx])).

For each of --n-episodes init_states we:
  1. perturb the gripper's initial Cartesian (x, y, z) per the preset,
  2. run a Cosmos-Policy rollout under the original task prompt,
...
NPZ schema (shared by positive.npz and negative.npz):
    ...
    episode_idx    : (N,) int32              (source rollout episode)
...
def main():
    ...
    ap.add_argument("--n-episodes", type=int, default=50,
                    help="number of init_states (N settings) to roll out; "
                         "capped by suite.get_task_init_states(task).shape[0]")
    ...
    ap.add_argument("--prompt", type=str, default=None,
                    help="task prompt; default = libero env's built-in description")
    ...
    _log(f"libero suite={args.suite} task={args.task_id}  "
         f"n_episodes={n_episodes}  max_env_steps={max_env_steps}")
    ...
    manifest = {
        ...
        "pairing": (
            ...
            "and rows are NOT 1-to-1 matched at identical states. Do NOT "
            "feed these directly to run_partition_svd_pairs*.py; instead "
            "run notebooks/lqr/svd/pair_inputs_by_similarity.py to produce "
        ...
        "drive_sources": [
            {"code": 0, "name": "gripper_xyz_perturbed",
             "desc": "env perturbed via RandomGripperXYZPerturbation, "
                     "then policy steps cleanly under PROMPT"},
        ],
        "sets": {
