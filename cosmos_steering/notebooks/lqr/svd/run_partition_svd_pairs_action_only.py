...
DiT block activations with a *paired observation* contrast — variant of
run_partition_svd_pairs_no_action.py that extracts activations from ONLY
the action slot (the inverse selection of run_partition_svd_pairs_no_action.py).

For LIBERO the DiT operates on `state_t = 9` slots:

    [0 .. min_cond-1]   = image / proprio conditioning slots (not denoised)
    min_cond + 0        = action               <-- ONLY slot extracted here
    min_cond + 1        = future_proprio       (skipped)
    min_cond + 2        = future_wrist         (skipped)
    min_cond + 3        = future_third_person  (skipped)
    min_cond + 4        = value                (skipped)

run_partition_svd_pairs.py extracts [min_cond .. state_t) (all 5 denoise slots).
run_partition_svd_pairs_no_action.py extracts [min_cond+1 .. state_t) (skips action).
This script extracts [min_cond .. min_cond+1) (action slot only) so the SVD basis
covers only the action subspace.

Because compute_jacobians_full.py, compute_contrastive_per_j.py, and the LQR
scripts all read `denoise_t_start` / `denoise_t_end` / `T_p_denoise` / `D_flat`
from config.json, this single change makes the downstream pipeline operate over
the action slot only — Jacobians map the action subspace to itself, and LQR
steering reads from / writes back to the action slot only.

Outputs are plug-in compatible with the existing
notebooks/lqr/jacobians/compute_jacobians_*.py and notebooks/lqr/*.py LQR
runners, just with the smaller D_flat (1/5 of the no-action variant).
...
DEFAULT_POS_NPZ = (
    ...
    "libero_10__task00__xyz_random_xlarge_3__seed42__pos_neg__paired/positive.npz"
)
DEFAULT_NEG_NPZ = (
    "/u/jhong7/Workspace/cosmos-policy/notebooks/lqr/inputs/policy_inputs/"
    "libero_10__task00__xyz_random_xlarge_3__seed42__pos_neg__paired/negative.npz"
)
...
def run_sketch(args):
    ...
    # LIBERO slot layout (state_t=9, min_cond=4):
    #   slot 4 = action            <-- ONLY slot extracted here
    #   slot 5 = future_proprio
    #   slot 6 = future_wrist
    #   slot 7 = future_third_person
    #   slot 8 = value
    denoise_t_start = min_cond
    denoise_t_end = min_cond + 1
    if denoise_t_end > state_t:
        raise ValueError(
            f"action slot out of range: min_cond={min_cond} state_t={state_t}"
        )
    ...
    H_p = H_lat // p_s
    W_p = W_lat // p_s
    T_p_denoise = denoise_t_end - denoise_t_start  # = 1 (action only)
...
def run_svd(args):
    ...
    print(f"action_slot_only={cfg.get('action_slot_only')}  "
    ...
    for p_idx, (l_start, l_end) in enumerate(partitions):
        ...
        for t_pos, t_id in enumerate(sel_t):
            ...
            # Action-only D_flat is much smaller than the full slot set, so
            # k_plus_p can exceed D_flat. In that case Halko/rSVD cannot
            # produce more than D_flat singular values; we exact-SVD the
            # stacked W matrix directly.
            if R_p < k_plus_p:
            ...
            # Cap k by D_flat too: with action-only the activation subspace
            # has rank <= D_flat (typically ~1280), which can be smaller than
            # k_target. The downstream LQR/jacobian code is robust to zero
            # padding past the effective rank.
            k_pre = min(k_target, R_p - p_over, k_rank, D_flat)
            print(f"  σ_max={sigma_max:.3e}  "
                  f"k_rank(τ_rel={TAU_REL:.0e})={k_rank}  D_flat={D_flat}  "
    ...
    for p_idx, (l_start, l_end) in enumerate(partitions):
        for t_pos, t_id in enumerate(sel_t):
            ...
            torch.save({
                ...
                "action_slot_only": cfg.get("action_slot_only", True),
