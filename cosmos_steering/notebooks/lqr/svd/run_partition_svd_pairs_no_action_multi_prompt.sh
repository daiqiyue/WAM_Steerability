#!/bin/bash
# Slurm launcher for run_partition_svd_pairs_no_action_multi_prompt.py —
# multi-prompt variant of run_partition_svd_pairs_no_action.sh.
#
# Reads the per-row prompt assignment from the NPZ (via `prompts` lookup +
# `config_idx` columns produced by collect_policy_inputs_object_pairs.ipynb)
# instead of taking a single --prompt CLI string. Suitable for a single SVD
# across rows generated under multiple prompts (e.g. the 3 object-pair scenes
# in libero_10__task00__object_pairs_pos_neg).
#
# Output is plug-in compatible with notebooks/lqr/jacobians/compute_jacobians_*.sh
# and the LQR shells (same config.json shape fields; only `prompt` -> `unique_prompts`
# + `prompt_rows_per_config` additions).
#
# Usage:
#   ./run_partition_svd_pairs_no_action_multi_prompt.sh
#   WORLD_SIZE=8 N=480 ./run_partition_svd_pairs_no_action_multi_prompt.sh
#   DRIVE_SOURCE=0 ./run_partition_svd_pairs_no_action_multi_prompt.sh
#   DRIVE_SOURCE=1 ./run_partition_svd_pairs_no_action_multi_prompt.sh
...
# NPZ pair must carry `prompts` (object array, lookup) and `config_idx`
# (per-row index into the lookup). The unified positive.npz / negative.npz
# produced by collect_policy_inputs_object_pairs.ipynb does.
POS_NPZ="${POS_NPZ:-/u/jhong7/Workspace/cosmos-policy/notebooks/lqr/inputs/policy_inputs/libero_10__task00__object_pairs_pos_neg/positive.npz}"
NEG_NPZ="${NEG_NPZ:-/u/jhong7/Workspace/cosmos-policy/notebooks/lqr/inputs/policy_inputs/libero_10__task00__object_pairs_pos_neg/negative.npz}" 
...
TAG="${TAG:-libero10_task00_object_pairs_pairs_no_action_multiprompt_ds${DRIVE_SOURCE}}"
...
PY_SCRIPT="$SCRIPT_DIR/run_partition_svd_pairs_no_action_multi_prompt.py"
...
echo "  prompt source:     per-row (from NPZ `prompts` + `config_idx`)"
...
SKETCH_JOB_NAME="cpspnamp_sketch_${RUN_TAG}"
...
SVD_JOB_NAME="cpspnamp_svd_${RUN_TAG}"
