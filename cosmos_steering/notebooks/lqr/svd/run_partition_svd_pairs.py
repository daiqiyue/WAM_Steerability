...
DiT block activations with a *paired observation* contrast (instead of the
prompt-swap contrast of run_partition_svd_cosmos_policy.py).

For each paired row n, two forward passes share the same prompt but use
*different observations*:

    run_one(prompt, obs_pos[n], n=n, sign=+1.0)     # distractors-absent
    run_one(prompt, obs_neg[n], n=n, sign=-1.0)     # distractors-present

The proprio in obs_pos[n] and obs_neg[n] is identical by construction (see
collect_policy_inputs_milk.ipynb), so the (pos - neg) contrast isolates the
*visual* effect of distractor presence at a matched robot pose.

Output is plug-in compatible with notebooks/lqr/jacobians/compute_jacobians_*.py:
V_part{p}_layers{a}-{b}_t{t}_k{k}.pt, svd_summary.pt, config.json — same
schema as the prompt-swap variant. Only `input_mode`, the npz path keys, and
the prompt field in config.json differ.
...
Notes
-----
The drive_source column (0=neg-drives, 1=pos-drives) in the paired npz files
records which env stepped during data collection. By default this script
ignores it (uses all rows). Pass --drive-source 0 or 1 to restrict the SVD
to one campaign.
"""
...
DEFAULT_PROMPT = "put both the milk and the tomato sauce in the basket"
DEFAULT_POS_NPZ = (
    "/u/jhong7/Workspace/cosmos-policy/notebooks/lqr/inputs/policy_inputs/"
    "libero_10__task00__milk_pos_neg/positive.npz"
)
DEFAULT_NEG_NPZ = (
    "/u/jhong7/Workspace/cosmos-policy/notebooks/lqr/inputs/policy_inputs/"
    "libero_10__task00__milk_pos_neg/negative.npz"
)
...
def parse_args():
    ...
    ap.add_argument("--prompt", type=str, default=DEFAULT_PROMPT,
                    help="single prompt used for BOTH positive and negative "
                         "forward passes (the contrast comes from obs only)")
    ap.add_argument("--pos-npz", type=Path, default=Path(DEFAULT_POS_NPZ),
                    help="paired-pos npz with keys primary_images, "
                         "wrist_images, proprios, drive_source")
    ap.add_argument("--neg-npz", type=Path, default=Path(DEFAULT_NEG_NPZ),
                    help="paired-neg npz with same row count and ordering "
                         "as --pos-npz")
    ap.add_argument("--drive-source", type=str, default="all",
                    help="restrict pairs by drive_source: 'all' | '0' (neg-drives) "
                         "| '1' (pos-drives). Default: all.")
    ap.add_argument("--N", type=int, default=-1,
                    help="number of paired rows to use (after drive-source "
                         "filter). -1 = use all available.")
...
# ====================================================================
# Observation loading — paired
# ====================================================================

def _filter_indices(drive_source_arr, drive_filter):
    """Return the row indices to keep, after applying --drive-source filter.

    drive_filter is 'all' | '0' | '1'.
    """
    if drive_filter == "all":
...
def load_paired_observations(pos_npz: Path, neg_npz: Path, drive_filter: str,
                              N_request: int):
    """Load matched (obs_pos[i], obs_neg[i]) pairs from two npz files.

    Both files must share row count and the (episode_idx, inference_idx,
    drive_source) columns so that row i in each file is a contrastive pair.

    Returns
    -------
    obs_pos_list, obs_neg_list : lists of obs dicts (same length = N_used)
    n_used : int
    n_avail_after_filter : int
    selected_rows : ndarray (n_used,) — original-row indices we used
    drive_source_used : ndarray (n_used,) — drive_source value per used row
    """
    pos = np.load(pos_npz)
    neg = np.load(neg_npz)

    # Sanity: shapes must match across pos/neg.
    ...
    # Pairing invariants — episode_idx and inference_idx must match row-for-row.
    if not np.array_equal(pos["episode_idx"], neg["episode_idx"]):
        raise ValueError(
            "episode_idx differs between pos and neg npz — pairing broken"
        )
    if not np.array_equal(pos["inference_idx"], neg["inference_idx"]):
        raise ValueError(
            "inference_idx differs between pos and neg npz — pairing broken"
        )
    if not np.array_equal(pos["drive_source"], neg["drive_source"]):
        raise ValueError(
            "drive_source differs between pos and neg npz — pairing broken"
        )
...
def run_sketch(args):
    ...
    # All downstream sizing uses N (= N_used); update args.N so rank-0's
    # config dump records the value actually used.
    args.N = N_used
    ...
    # Single prompt -> precompute T5 once (avoids any race on
    # save_t5_text_embeddings_cache across ranks).
    _ = get_t5_embedding_from_cache(args.prompt)
    ...
    for j_local, n in enumerate(idx):
        tp = time.time()
        # Same prompt, paired obs: sign +1 on pos (distractors-absent),
        # sign -1 on neg (distractors-present). The (pos - neg) contrast
        # therefore isolates the *visual* effect of distractor presence at
        # an identical-proprio matched pose.
        run_one(args.prompt, obs_pos_list[n], n=n, sign=+1.0)
        run_one(args.prompt, obs_neg_list[n], n=n, sign=-1.0)
    ...
    if rank == 0:
        ...
        cfg_out = {
            ...
            "input_mode": "paired_obs_fixed_prompt",
            "W_dtype": "bfloat16",
            "mu_dtype": "float32",
        }
