...
run_partition_svd_pairs.py that EXCLUDES the action slot when extracting
activations.

For LIBERO the DiT operates on `state_t = 9` slots:

    [0 .. min_cond-1]   = image / proprio conditioning slots (not denoised)
    min_cond + 0        = action               <-- EXCLUDED here
    min_cond + 1        = future_proprio
    min_cond + 2        = future_wrist
    min_cond + 3        = future_third_person
    min_cond + 4        = value

The original run_partition_svd_pairs.py extracts slots [min_cond .. state_t),
i.e. all five denoising slots. This script shifts the start of the extraction
by +1 so the action slot is skipped, leaving 4 slots (future_proprio,
future_wrist, future_third_person, value).

Because compute_jacobians_full.py, compute_contrastive_per_j.py, and the LQR
notebooks all read `denoise_t_start` / `denoise_t_end` / `T_p_denoise` /
`D_flat` from config.json, this single change makes the downstream pipeline
operate over the same non-action slots automatically — Jacobians and LQR
steering inputs / outputs all skip the action slot.

Outputs are plug-in compatible with the existing
notebooks/lqr/jacobians/compute_jacobians_*.py and notebooks/lqr/*.ipynb,
just with the smaller D_flat. The svd_summary.pt continues to contain the
global average contrastive `c_means` (averaged over all paired rows /
inferences). To additionally compute per-inference-index contrastive vectors
(c_means_per_j.pt), run notebooks/lqr/svd/compute_contrastive_per_j.py
against this script's --out-dir afterwards — it reads denoise_t_start from
config.json and will respect the same action-slot exclusion.
...
def parse_args():
    ...
    ap.add_argument("--include-action-slot", dest="exclude_action_slot",
                    ...
                    help="legacy behavior: include the action slot "
                         "(matches run_partition_svd_pairs.py).")
...
def run_sketch(args):
    ...
    #   slot 4 = action            <-- skipped iff exclude_action_slot
    ...
    H_p = H_lat // p_s
    W_p = W_lat // p_s
    T_p_denoise = denoise_t_end - denoise_t_start  # p_t == 1
    ...
    if rank == 0:
        ...
        cfg_out = {
            ...
            "input_mode": (
                "paired_obs_fixed_prompt_no_action"
                if args.exclude_action_slot
                else "paired_obs_fixed_prompt"
            ),
            "W_dtype": "bfloat16",
            "mu_dtype": "float32",
        }
