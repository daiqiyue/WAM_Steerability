...
# with the action slot excluded from activation extraction.
#
# Variant of run_partition_svd_pairs.sh: the only difference is that the
# SVD (and therefore every downstream consumer: compute_contrastive_per_j,
# compute_jacobians_full, the LQR notebooks) operates on slots
#   [min_cond + 1 .. state_t) = [5 .. 9) for LIBERO,
# skipping the action slot.
#
...
# operate on the same non-action slots (they read denoise_t_start / denoise_t_end
...
#   DRIVE_SOURCE=0 ./run_partition_svd_pairs_no_action.sh    # neg-drives rows only
#   DRIVE_SOURCE=1 ./run_partition_svd_pairs_no_action.sh    # pos-drives rows only
...
POS_NPZ="${POS_NPZ:-/u/jhong7/Workspace/cosmos-policy/notebooks/lqr/inputs/policy_inputs/libero_10__task00__noise_high_4_pos_neg/positive.npz}"
NEG_NPZ="${NEG_NPZ:-/u/jhong7/Workspace/cosmos-policy/notebooks/lqr/inputs/policy_inputs/libero_10__task00__noise_high_4_pos_neg/negative.npz}"
DRIVE_SOURCE="${DRIVE_SOURCE:-all}"

# Set INCLUDE_ACTION=1 to revert to the original behavior (action slot included).
INCLUDE_ACTION="${INCLUDE_ACTION:-0}"

# TAG="${TAG:-libero10_task00_milk_pairs_no_action_ds${DRIVE_SOURCE}}"
TAG="${TAG:-libero10_task00_noise_high_4_pairs_no_action_ds${DRIVE_SOURCE}}"
...
SKETCH_JOB_NAME="cpspna_sketch_${RUN_TAG}"
...
SVD_JOB_NAME="cpspna_svd_${RUN_TAG}"
