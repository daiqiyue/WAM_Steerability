#!/bin/bash
# Launcher for compute_jacobians_text.py — sbatch-array workers, then a
# login-node merge once all workers have left the queue. Mirrors the
# updated run_jacobians_full.sh.
#
# Stage 1: sbatch array with WORLD_SIZE tasks (--gpus-per-task=1). Each
#          array element derives its (t, l) slice from --rank/--num-shards
#          and writes shard_rank<R>.pt.
# Stage 2: poll squeue until all array tasks are gone, then run --phase merge
#          directly in this shell. Merge needs no GPU and no model load.
#          Login-node merge sidesteps the DependencyNeverSatisfied trap that
#          bites when a worker crashes during interpreter teardown (non-zero
#          exit AFTER shards saved — observed for text-jacobian rank 1 in
#          job 2289367).
#
# This script BLOCKS while workers run. Use tmux / screen / nohup to detach.
# If you Ctrl-C the wait loop, workers keep running; resume the merge with:
#   MERGE_ONLY=1 PROMPT="<same prompt>" ./run_jacobians_text.sh
#
# Usage:
#   ./run_jacobians_text.sh
#   WORLD_SIZE=8 SVD_DIR=/path/to/svd_run \
#       PROMPT="put both the milk and the tomato sauce in the basket" \
#       ./run_jacobians_text.sh
#   INPUTS_NPZ=/path/to/other.npz OBS_INDEX=12 ./run_jacobians_text.sh
#   MERGE_ONLY=1 ./run_jacobians_text.sh   # skip workers, just re-merge shards
...
# User-configurable
# =========================================================================
WORLD_SIZE="${WORLD_SIZE:-8}"        # ranks (one GPU each)
...
MODE="${MODE:-vjp_no_retain}"           # 'vjp' | 'vjp_no_retain'
# Default is vjp_no_retain because Cosmos-Policy's DiT uses selective
# activation checkpointing (SAC), which forbids multiple backward passes
# through the same checkpoint cycle. 'vjp' would crash with
# "Trying to backward an extra time".
...
PY_SCRIPT="$SCRIPT_DIR/compute_jacobians_text.py"
...
RUN_TAG="B_text_full__${PROMPT_TAG}__${MODE}__v${V_DTYPE}"
...
if [[ "$MERGE_ONLY" == "1" || "$MERGE_ONLY" == "true" ]]; then
    ...
    WORKER_JOB_NAME="cps_jactext_${PROMPT_TAG}"
