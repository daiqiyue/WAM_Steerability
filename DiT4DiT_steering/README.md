...
To branch a saved inference into matched raw and ActAdd continuations, use
`interpretability/replay_snapshot_actadd.py`. The checked-in Gaussian-noise
...
noise-generator state when present, and policy seed. The comparison video labels every
inference and displays the clean view, exact agent/wrist model inputs, and
end-effector displacement. The accompanying JSON records per-axis and L2 EEF
displacement, trajectory length, success, executed inference/action counts, and
whether the first model input was reproduced exactly. The selected
initial-gripper-position episode-6/inference-10 experiment uses:

```bash
sbatch run_dit4dit_gripper_snapshot_actadd_comparison.sbatch
```

Both wrappers use unit-norm reconstructed contrastive directions, `alpha=1`,
and every block/denoising step; all three choices are explicit command-line
arguments in the Python entry point.
